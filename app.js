...
function resetColors() {
    ...
    for (i = 0; i < tipButtons.length - 1; i++) {
        if (tipButtons[i].style.backgroundColor === "var(--string-cyan)") {
...
billTotal.addEventListener("keydown", function(e) {
    ...
    if (keyID === 8) {
        if (this.value.length > 1) {
            this.value = this.value.subtring(0, this.value.length);
        }
    }
});
...
resetButton.addEventListener("click", function() {
    ...
    peopleTotal.value = "";
    peopleTotalValue = 0;
    tipPerPerson.textContent = "Rp0,00";
    tipPercentValue = 0;
    totalPerPerson.textContent = "Rp0,00";
